...
class HighwayTransform extends stream.Transform {
    ...
    constructor(
        protected c: Client,
        protected obj: HighwayUploadExt,
    ) {
        super();
        // if (!obj.ticket)
...
/** highway上传数据 (只能上传流) */
export function highwayUpload(
    this: Client,
    readable: stream.Readable,
    obj: HighwayUploadExt,
    ip: string | number = 'htdata3.qq.com',
    port: number = 80,
): Promise<pb.Proto> {
    ip = int32ip2str(ip);
    ...
    return new Promise((resolve, reject) => {
        ...
        const socket = net.connect(port as number, ip as string, () => {
